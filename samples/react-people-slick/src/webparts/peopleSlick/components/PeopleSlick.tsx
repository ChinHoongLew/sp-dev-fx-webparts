            ...
            {this.state.listItems.map((item: CarousalItem) => {
              return (
                <div className={styles.carousalItem} key={item.Id}>
                   <p className={styles.profile}><img width={`${this.props.photoWidth}`} src={`${this.props.rootSiteURL}/_layouts/15/userphoto.aspx?size=L&accountname=${item.Email.EMail}`} /></p><p className={styles.title}>{item.Email.Title}</p>
